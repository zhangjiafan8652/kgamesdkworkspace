...
public class AsyncHttpConnection {
	...
	public void Opost(final String url, final ParamsWrapper params,
		...
		new Thread() {
			public void run() {
				...
				try {
					...
					callback.onResponse(null, AsyncHttpConnection.this.result,
							Ourl, Code);
					...
					try {
						URL Ourl = new URL(url.toString());
						callback.onResponse(null, "timeout", Ourl, 1002);
					} catch (MalformedURLException e1) {
						e1.printStackTrace();
					}
	...
	public void Oget(final String url, final ParamsWrapper params,
			final ResponseCallback callback) {
		new Thread() {
			public void run() {
