...
import com.yayawan.sdk.jfxml.Weibologinxml_po;
import com.yayawan.sdk.jfxml.Yinlianpay_xml_po;
...
public class Yinlian extends BaseView implements YayaWanUserCallback {
			...
			case BILLRESULT:
				...
				if (mBillResult != null) {
					if (mBillResult.success == 1) {
						// 第二次确认操作失败
						if (YayaWan.mPaymentCallback!=null) {
							YayaWan.mPaymentCallback.onError(0);
							YayaWan.mPaymentCallback=null;
						}
						...
						if (YayaWan.mPaymentCallback!=null) {
							YayaWan.mPaymentCallback.onSuccess(AgentApp.mUser, AgentApp.mPayOrder, 1);
							YayaWan.mPaymentCallback=null;
						}
						// 支付操作成功等待到账
						//YayaWan.mPaymentCallback.onSuccess(AgentApp.mUser, AgentApp.mPayOrder, 1);
		...
		wv_mWeiboview.setWebViewClient(new WebViewClient() {
			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {
				...
				if (url.startsWith("yayapayment://success?status=0")) {
					// YayaWan.mPaymentCallback.onSuccess(paramUser, paramOrder,
					// paramInt);
					checkOrder();
				}else {
					view.loadUrl(url);
