			...
			public void onResponse(int resultCode, String resultDesc,
				...
				case ResultCode.LOGIN_SUCCESS:
					...
					BDGameSDK.queryLoginUserAuthenticateState(
							getApplicationContext(), new IResponse<Integer>() {

								@Override
								public void onResponse(int resultCode,
										String resultDesc, Integer extraData) {

									Log.i("queryLoginUserAuthenticateState",
											"resultCode" + resultCode + "==" + extraData);
									Log.i("login", "login sucess");
									GameActivity.uid = BDGameSDK.getLoginUid(); // TODO
									// 保存登陆后获取的uid到调用支付API时使用
				...
				Toast.makeText(getApplicationContext(), hint, Toast.LENGTH_LONG)
						.show();
			}
		});
	}
