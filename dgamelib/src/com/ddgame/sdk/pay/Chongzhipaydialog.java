...
import com.ddgame.sdk.bean.BillResult;
import com.ddgame.sdk.bean.ConfirmPay;
...
import com.ddgame.sdk.utils.CardInfo;
import com.ddgame.sdk.utils.CardNumTextWatcher;
import com.ddgame.sdk.utils.DialogUtil;
import com.ddgame.sdk.utils.ToastUtil;
import com.ddgame.sdk.xml.GetAssetsutils;
import com.ddgame.sdk.xml.Loginpo_listviewitem;
...
public class Chongzhipaydialog extends Basedialogview  implements
KgameSdkPaymentCallback{
		...
		public void handleMessage(Message msg) {
			...
			case FIRSTRESULT:
				...
				if (mFirstResult.success == 1) {
					...
					// 操作失败
					onError(0);
					ToastUtil.showError(mActivity, mFirstResult.error_msg);
				...
				//	 System.out.println("这是支父密码:"+ mCardPassText);
					/*new Thread() {
			...
			case CONFIRMRESULT:
				if (mConfirmPay != null) {
					if (mConfirmPay.success == 1) {
						...
						onError(0);
						ToastUtil.showError(mActivity, mConfirmPay.error_msg);
					} else if (mConfirmPay.success == 0) {
						
						DialogUtil.showDialog(mActivity, "支付结果确认中...");
	...
	public Chongzhipaydialog(Activity activity) {
		super(activity);
	...
	public void createDialog(final Activity mActivity) {
		...
		machineFactory.MachineView(rl_content, 650, 600, mLinearLayout,2,25);
		...
		machineFactory.MachineView(ll_content, 650, 600, 0, mRelativeLayout,
				20, 0, 20, 0, 100);
		...
		ll_mMoney.setBackgroundDrawable(GetAssetsutils
	...
	private void initlogic() {
		...
		if (AgentApp.mentid == 11) {
			mCardNum.setText(AgentApp.mCardInfos.get(CardInfo.YIDONGCARDNUM));
			mCardPassword.setText(AgentApp.mCardInfos
					.get(CardInfo.YIDONGCARDPASS));
		} else if (AgentApp.mentid == 12) {
			mCardNum.setText(AgentApp.mCardInfos.get(CardInfo.LIANTONGCARDNUM));
			mCardPassword.setText(AgentApp.mCardInfos
					.get(CardInfo.LIANTONGCARDPASS));
		} else if (AgentApp.mentid == 15) {
			mCardNum.setText(AgentApp.mCardInfos.get(CardInfo.DIANXINCARDNUM));
			mCardPassword.setText(AgentApp.mCardInfos
					.get(CardInfo.DIANXINCARDPASS));
		}
		...
		mPaynow.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(mCardNumText)) {
					Toast.makeText(mActivity, "请输入卡号", Toast.LENGTH_SHORT)
							.show();
				} else if (mCardNumText.length() < 10) {
					Toast.makeText(mActivity, "卡号不能小于10位", Toast.LENGTH_SHORT)
							.show();
				} else if (mCardNumText.length() > 20) {
					Toast.makeText(mActivity, "卡号不能大于20位", Toast.LENGTH_SHORT)
							.show();
				} else if ("".equals(mCardPassText)) {
					Toast.makeText(mActivity, "请输入密码", Toast.LENGTH_SHORT)
							.show();
				} else if (mCardPassText.length() < 8) {
					Toast.makeText(mActivity, "密码不能小于8位", Toast.LENGTH_SHORT)
							.show();
				} else if (mCardPassText.length() > 20) {
					Toast.makeText(mActivity, "密码不能大于20位", Toast.LENGTH_SHORT)
							.show();
				} else {
					// 进入支付流程
					DialogUtil.showDialog(mActivity, "正在请求支付...");
					mPaynow.setEnabled(false);
					mCurrentMoney=et_mMoney.getText().toString().trim();
					if (AgentApp.mentid == 11) {
						AgentApp.mCardInfos.put(CardInfo.YIDONGCARDNUM,
								mCardNumText);
						AgentApp.mCardInfos.put(CardInfo.YIDONGCARDPASS,
								mCardPassText);
					} else if (AgentApp.mentid == 12) {
						AgentApp.mCardInfos.put(CardInfo.LIANTONGCARDNUM,
								mCardNumText);
						AgentApp.mCardInfos.put(CardInfo.LIANTONGCARDPASS,
								mCardPassText);
					} else if (AgentApp.mentid == 15) {
						AgentApp.mCardInfos.put(CardInfo.DIANXINCARDNUM,
								mCardNumText);
						AgentApp.mCardInfos.put(CardInfo.DIANXINCARDPASS,
								mCardPassText);
					}
					/*new Thread() {
						...
						public void run() {
							try {
								mFirstResult = ObtainData.payment(mActivity,
