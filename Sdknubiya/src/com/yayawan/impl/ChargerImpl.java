...
public class ChargerImpl implements YYWCharger {
	...
	public void createOrder(final Activity paramActivity) {
		...
		requestParams.addBodyParameter("game_uid",  GameSdk.getLoginUid());
		...
		httpUtil.send(HttpMethod.POST, ViewConstants.unionmakeorder,requestParams,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						// TODO Auto-generated method stub
						disprogress();
						Yayalog.loger("返回的数据："+arg0.result);
						try {
							JSONObject obj = new JSONObject(arg0.result);
							int err_code = obj.optInt("err_code");
							if (err_code == 0) {
								JSONObject data = obj.getJSONObject("data");
								YaYawanconstants.sign = obj.getString("sign");
								orderId = data.optString("id");
								
								JSONObject params = obj.getJSONObject("params");
								
								YaYawanconstants.pay_data_timestamp=params.optString("data_timestamp");
								YaYawanconstants.pay_amount=params.optString("amount");
								YaYawanconstants.pay_cp_order_id=params.optString("cp_order_id");
								YaYawanconstants.pay_product_des=params.optString("product_des");
								YaYawanconstants.pay_product_name=params.optString("product_name");
								//YaYawanconstants.pay_data_timestamp=params.optString("data_timestamp");
